...
accidents_tbl_2 %>% 
    ...
    # there are many different cases, some has number at the end of flight, some has (xyz), some has two flights concatnated with `and` however the `and` sometimes is not separated from previous and next word, and also the `and` can be found in words such as Poland, Midland, etc. 
