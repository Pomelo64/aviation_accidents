...
accidents_tbl_2 %>% 
    ...
    # there are many different cases, some has number at the end of flight, some has (xyz), some has two flights concatnated with `and` however the `and` sometimes is not separated from previous and next word, and also the `and` can be found in words such as Poland, Midland, etc. At the end, I assumed that when there are two airlines, the first airline is responsible one, not a fair assumption tho!     
    mutate(incident_flag = str_detect(string = incident, pattern = "\\(" )) %>% 
    mutate(airline = ifelse(incident_flag, str_split(string = incident, pattern = "\\(", simplify = T)[,1] ,
                            str_split(string = incident, pattern = "\\d", simplify = T)[,1] )) %>%
    
    separate(col = airline, into = c("airline","residual_airline"),sep = "[:digit:]" , remove = T , fill = "right") %>%
    separate(col = airline , into = c("airline","residual_airline") , sep = "' and'" , remove = T , fill = "right" ) %>% 
     mutate(incident_flag_2 = str_detect(string = airline , pattern = "and\\s" )) %>% 
    mutate(incident_flag_3 = str_detect(string = airline , pattern = "land")) %>% 
    mutate(airline = case_when(
        incident_flag_2 & !incident_flag_3 ~ str_split(string = airline , pattern = "and\\s" , simplify = T)[,1] , 
        TRUE ~ airline
    )) %>% 
    ...
    mutate(airline = case_when( 
        ...
        str_detect(string = airline ,
                   pattern = "Air Vietnam") ~ "Air Vietnam", 
        str_detect(string = airline ,
    ...
    # column selection 
    select_at(.vars = names(.)[!str_detect(names(.),"_flag")] ) %>% 
    select_at(.vars = names(.)[!str_detect(names(.),"residual_")] ) %>% 

    select(-nonFlying_fatalities, -notes, -citation, -rest, - location_pieces) %>% 
    select(id , everything())
    
    # data cleaning and feature engineering : end
